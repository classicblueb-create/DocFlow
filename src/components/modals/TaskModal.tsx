...
export function TaskModal({ isOpen, onClose, onSave, initialTask, categories = [] }: TaskModalProps) {
  ...
  const [lineStatus, setLineStatus] = useState<'idle' | 'sending' | 'sent' | 'error'>('idle');
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialTask) {
        ...
        setPriority('ปานกลาง (Medium)'); setPrice('');
      ...
      setIsGenerating(false); setLineStatus('idle');
    }
  }, [isOpen, initialTask]);
  ...
  const handleSave = async () => {
    ...
    // ส่ง LINE notification เมื่อมีการ assign ใหม่หรือเปลี่ยน assignee
    if (assignee && assignee !== prevAssignee) {
      ...
      try {
        ...
        setLineStatus(res.ok ? 'sent' : 'error');
      } catch {
        setLineStatus('error');
      }
    }
            ...
            {assignee && (
              <div className="rounded-xl border border-indigo-100 bg-indigo-50/50 p-4 space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-indigo-800">ลิงก์ไฟล์งาน (สำหรับแจ้ง LINE)</span>
                </div>
                <input
                  type="url"
                  value={fileUrl}
                ...
                {assignee !== prevAssignee && (
                  <p className="text-[11px] text-indigo-600 flex items-center gap-1">
                    ...
                    จะส่งแจ้งเตือน LINE กลุ่มอัตโนมัติเมื่อบันทึก
                  </p>
                )}
                {lineStatus === 'sent' && <p className="text-[11px] text-emerald-600 font-semibold">✓ ส่ง LINE แจ้งเตือนสำเร็จ</p>}
                {lineStatus === 'error' && <p className="text-[11px] text-red-500">⚠ ส่ง LINE ไม่สำเร็จ (ตรวจสอบ LINE_NOTIFY_TOKEN)</p>}
              </div>
            )}
