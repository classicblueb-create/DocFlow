                  ...
                  <div className="mt-0.5 md:mt-1 space-y-0.5">
                    {dayTasks.filter(t => t.endDate === dateStr).map((task, idx) => (
                      <a key={idx} href="#" onClick={e => e.preventDefault()}
                        className="block text-[7px] md:text-[9px] bg-red-500/15 text-red-700 px-0.5 py-0.25 md:px-1 md:py-0.5 rounded truncate border border-red-500/20 font-bold hover:bg-red-500/25 transition-colors">
                        {task.name}
                      </a>
                    ))}
                    {dayTasks.filter(t => t.startDate === dateStr && t.endDate !== dateStr).map((task, idx) => (
                      <div key={idx} className="text-[7px] md:text-[9px] bg-indigo-500/15 text-indigo-700 px-0.5 py-0.25 md:px-1 md:py-0.5 rounded truncate border border-indigo-500/20 font-bold">
                        ▶ {task.name}
                      </div>
                    ))}
                    {dayEvents.map((ev, idx) => (
                      <a key={idx} href={ev.htmlLink || '#'} target="_blank" rel="noopener noreferrer"
                        className="block text-[7px] md:text-[9px] bg-emerald-500/15 text-emerald-700 px-0.5 py-0.25 md:px-1 md:py-0.5 rounded truncate border border-emerald-500/20 font-bold hover:bg-emerald-500/25 transition-colors">
